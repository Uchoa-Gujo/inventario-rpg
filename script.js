...
function normalizarUsuario(usuario) {
  return String(usuario || '').trim().toLowerCase().replace(/\s+/g, '')
}
...
function traduzirErro(error) {
  ...
  if (msg.includes('Password should be at least 6 characters')) return 'A senha precisa ter pelo menos 6 caracteres.'
  if (msg.toLowerCase().includes('email rate limit exceeded')) return 'Limite temporário do Supabase atingido. Desative confirmação por email no painel do Supabase.'
  if (msg.includes('Signup is disabled')) return 'O cadastro está desativado no Supabase.'
...
function mostrarCadastroTela() {
  if (registerFormBox) registerFormBox.classList.remove('hidden')
...
function setSaveStatus(texto) {
  if (saveStatus) saveStatus.textContent = texto
}
...
function updateSlotsLockButton() {
  ...
  if (toggleSlotsLockBtn) toggleSlotsLockBtn.textContent = slotsLocked ? '🔒 Slots' : '🔓 Slots'
}
...
function syncSelectedEditors() {
  ...
  if (itemNameInput) itemNameInput.value = item.nome ?? ''
  if (itemDescInput) itemDescInput.value = item.descricao ?? ''
}
...
function renderCharacterAvatar(imageUrl = null) {
  if (!characterAvatarPreview) return
...
function syncBrandEditors() {
  if (brandTitleInput && brandTitle) brandTitleInput.value = brandTitle.textContent
  if (brandSubtitleInput && brandSubtitle) brandSubtitleInput.value = brandSubtitle.textContent
}
...
function openDeleteModal(item) {
  ...
  if (confirmModalText) confirmModalText.textContent = `Tem certeza que deseja excluir "${item.nome}"?`
...
function renderLeftCardButtons() {
  ...
  leftPanelBody.querySelectorAll('.dock-card').forEach((card, index, arr) => {
    ...
    if (downBtn) downBtn.disabled = index === arr.length - 1
    if (toggleBtn) toggleBtn.textContent = card.classList.contains('collapsed') ? '+' : '—'
  })
}

function moveCard(card, direction) {
  const sibling = direction === 'up' ? card.previousElementSibling : card.nextElementSibling
  if (!sibling || !leftPanelBody) return
...
function setupLeftCards() {
  ...
  leftPanelBody.querySelectorAll('.dock-card').forEach((card) => {
    ...
    if (upBtn) upBtn.addEventListener('click', () => moveCard(card, 'up'))
    if (downBtn) downBtn.addEventListener('click', () => moveCard(card, 'down'))
    if (toggleBtn) {
...
function renderKeybinds() {
  ...
  if (controlsConfigPanel) controlsConfigPanel.classList.toggle('hidden', controlsPanelCollapsed)
}
...
function rotateSelectedBlueGroup() {
...
function renderPanelButtons() {
  if (toggleLeftPanelBtn && leftPanel) toggleLeftPanelBtn.textContent = leftPanel.classList.contains('collapsed') ? '+' : '—'
  if (toggleRightPanelBtn && rightPanel) toggleRightPanelBtn.textContent = rightPanel.classList.contains('collapsed') ? '+' : '—'
  renderLeftCardButtons()
}

function render() {
  if (!inventoryGrid || !inventoryItemsLayer || !shelfArea) return
  ...
  node.addEventListener('mousedown', (event) => {
    ...
    startDragSession({
      ...
      onDropCommit: (x, y, upEvent) => {
        ...
        if (droppedEquipSlot) {
          if (equipItemToSlot(item, droppedEquipSlot)) {
            playSound('equip')
            agendarAutoSave()
          } else {
            playSound('error')
...
function makeShelfItemDrag(node, item) {
  node.addEventListener('mousedown', (event) => {
    ...
    startDragSession({
      ...
      onClick: (upEvent) => {
        if (upEvent.shiftKey) {
          if (moveItemToInventoryFirstFree(item)) {
            playSound('drop')
            agendarAutoSave()
          } else playSound('error')
          setSelection({ itemId: item.id })
...
function resetLayout() {
  ...
  if (brandTitle) brandTitle.textContent = 'Inventário RPG'
  if (brandSubtitle) brandSubtitle.textContent = 'Inventário modular com grupos duplos, mochila livre e cadastro de itens'
...
function coletarEstadoInventario() {
  return structuredClone({
    ...
    strength: strengthInput?.value || '8',
    brandTitle: brandTitle?.textContent || 'Inventário RPG',
    brandSubtitle: brandSubtitle?.textContent || '',
    avatarImage: characterAvatarPreview?.querySelector('img')?.src || null
  })
}
...
function aplicarEstadoInventario(dados) {
  ...
  blueGroups = Array.isArray(dados.blueGroups) ? structuredClone(dados.blueGroups) : structuredClone(initialBlueGroups)
  backpackModules = Array.isArray(dados.backpackModules) ? structuredClone(dados.backpackModules) : []
  selectedItemId = dados.selectedItemId ?? null
  selectedBackpackId = dados.selectedBackpackId ?? null
  selectedBlueGroupId = dados.selectedBlueGroupId ?? null
  slotsLocked = typeof dados.slotsLocked === 'boolean' ? dados.slotsLocked : true
  controlsPanelCollapsed = typeof dados.controlsPanelCollapsed === 'boolean' ? dados.controlsPanelCollapsed : false
  soundEnabled = typeof dados.soundEnabled === 'boolean' ? dados.soundEnabled : true
  keybinds = dados.keybinds ? structuredClone(dados.keybinds) : {
    rotateRight: 'r',
    rotateLeft: 'q',
    moveLeft: 'arrowleft',
    moveRight: 'arrowright',
  ...
  shapeMatrix = Array.isArray(dados.shapeMatrix)
    ? structuredClone(dados.shapeMatrix)
    : Array.from({ length: GRID_ROWS }, () => Array.from({ length: GRID_COLS }, () => false))

  if (strengthInput && dados.strength != null) strengthInput.value = dados.strength
  if (brandTitle) brandTitle.textContent = dados.brandTitle || 'Inventário RPG'
  if (brandSubtitle) brandSubtitle.textContent = dados.brandSubtitle || ''
  if (dados.theme) document.body.dataset.theme = dados.theme
  renderCharacterAvatar(dados.avatarImage || null)
...
async function entrarNoPersonagem() {
  abrirApp()
  ...
  if (dadosSalvos) {
    ...
    fillRectShape(2, 2)
    render()
    ultimoEstadoSalvo = structuredClone(coletarEstadoInventario())
  }
...
if (showLoginTab) showLoginTab.addEventListener('click', mostrarLoginTela)
if (showRegisterTab) showRegisterTab.addEventListener('click', mostrarCadastroTela)
if (backToLoginBtn) backToLoginBtn.addEventListener('click', mostrarLoginTela)
...
if (registerConfirmarSenhaInput) {
...
if (rotateRightBtn) rotateRightBtn.addEventListener('click', () => rotateSelected('right'))
if (rotateLeftBtn) rotateLeftBtn.addEventListener('click', () => rotateSelected('left'))

if (toggleSlotsLockBtn) {
...
if (unequipBtn) unequipBtn.addEventListener('click', unequipSelectedItem)
if (moveToShelfBtn) moveToShelfBtn.addEventListener('click', moveSelectedItemToShelf)
if (deleteItemBtn) deleteItemBtn.addEventListener('click', deleteSelectedItemPrompt)
if (resetBtn) resetBtn.addEventListener('click', resetLayout)

if (confirmDeleteBtn) confirmDeleteBtn.addEventListener('click', deletePendingItem)
...
if (saveBrandTextBtn) {
  saveBrandTextBtn.addEventListener('click', () => {
    if (brandTitle) brandTitle.textContent = brandTitleInput.value.trim() || 'Inventário RPG'
    if (brandSubtitle) brandSubtitle.textContent = brandSubtitleInput.value.trim() || ''
    playSound('click')
    agendarAutoSave()
  })
}

if (addBackpackBtn) addBackpackBtn.addEventListener('click', addBackpackFromShape)
if (removeBackpackBtn) removeBackpackBtn.addEventListener('click', removeSelectedBackpack)
if (clearShapeBtn) {
...
if (saveItemTextBtn) saveItemTextBtn.addEventListener('click', saveSelectedItemText)
if (addItemBtn) addItemBtn.addEventListener('click', addSimpleItem)
...
window.addEventListener('keydown', (event) => {
  ...
  } else if (key === keybinds.rotateLeft) {
    event.preventDefault()
    rotateSelected('left')
  } else if (key === keybinds.moveLeft) {
    event.preventDefault()
    moveSelectedBy(-1, 0)
  } else if (key === keybinds.moveRight) {
    event.preventDefault()
    moveSelectedBy(1, 0)
  } else if (key === keybinds.moveUp) {
    event.preventDefault()
    moveSelectedBy(0, -1)
  } else if (key === keybinds.moveDown) {
    event.preventDefault()
    moveSelectedBy(0, 1)
  } else if (key === 'delete') {
    event.preventDefault()
    deleteSelectedItemPrompt()
  }
})
